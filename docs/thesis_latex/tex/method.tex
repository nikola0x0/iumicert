...
% TODO: Expand methodology to 16 pages as per REPORT.md outline
% TODO: Include the following sections as outlined in REPORT.md:
% 1. Requirement Analysis (partially covered)
% 2. System Architecture & Design (partially covered)
...
% TODO: Add remaining sections to meet 16-page requirement as per REPORT.md:
% - System Architecture & Design

\section{System Architecture and Design}

The IU-MiCert system implements a multi-component architecture designed to support both issuance and verification of academic micro-credentials with verifiable provenance:

\subsection{Core Components}

\begin{enumerate}
    \item \textbf{Verkle Tree Manager}: Efficient storage/verification of micro-credentials using Ethereum's \texttt{go-verkle} library
    \item \textbf{Data Pipeline}: Academic data generation, conversion, and processing workflow
    \item \textbf{Blockchain Integration}: Smart contracts deployed on Ethereum for publishing term roots
    \item \textbf{Web Interfaces}: Next.js-based dashboards for issuer operations and verification
    \item \textbf{CLI Tools}: 15+ commands for complete workflow management (init, data generation, proof generation, verification, publishing)
\end{enumerate}

\subsection{Verkle Tree Implementation}

The system utilizes a \textbf{single Verkle tree per academic term}, where each tree contains all course completions for that term. Key features include:

\begin{itemize}
    \item Course data hashed to 32-byte values stored as tree leaves
    \item Deterministic key construction: \texttt{studentID:termID:courseID}
    \item 32-byte IPA (Inner Product Argument) proofs per course
    \item Enables selective disclosure: prove specific courses without revealing all
\end{itemize}

% TODO: Add detailed algorithmic representations and formalizations
